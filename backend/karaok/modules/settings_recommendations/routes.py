...
def _feature_required(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not application.SETTINGS_RECOMMENDATIONS_ENABLED:
...
def _require_user(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        return application.require_auth("user")(view)(*args, **kwargs)
...
def profile_metadata():
    return jsonify(service.get_profile_metadata())
...
@_require_user
def list_profiles():
    return jsonify(service.list_amplifier_profiles(g.user_id))
...
@_require_user
def create_profile():
    result = _service_call(service.create_amplifier_profile, g.user_id, _body())
...
def update_profile(profile_id: int):
    result = _service_call(
        service.update_amplifier_profile,
        g.user_id,
        profile_id,
        _body(),
    )
...
def delete_profile(profile_id: int):
    result = _service_call(service.delete_amplifier_profile, g.user_id, profile_id)
    if isinstance(result, tuple):
        return result
...
def get_recommendation(recommendation_id: int):
    result = _service_call(
        service.get_owned_recommendation,
        g.user_id,
        recommendation_id,
    )
    if isinstance(result, tuple):
...
def apply_recommendation(recommendation_id: int):
    result = _service_call(
        service.mark_recommendation_applied,
        g.user_id,
        recommendation_id,
    )
    if isinstance(result, tuple):
